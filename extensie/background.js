...
async function bewaarKandidaat(d){
  const notitie = ['Toegevoegd via LinkedIn-extensie',
  ...
  const rij = {
    ...
    woonplaats: d.plaats || '', bron: 'LinkedIn', campagne: '',
...
async function bewaarSales(d){
  const notitie = [
    d.naam ? `Contactpersoon: ${d.naam}${d.functie ? ` (${d.functie})` : ''}` : '',
    d.email ? `E-mail: ${d.email}` : '',
    d.telefoon ? `Tel: ${d.telefoon}` : '',
    d.notitie || '',
    d.url ? `LinkedIn: ${d.url}` : ''
  ].filter(Boolean).join(' · ');
  const rij = {
    id: uid('lr'), bedrijf: d.bedrijf, plaats: d.plaats || '',
    functies: d.functie || 'via LinkedIn', vacatures: 1, bron: 'handmatig',
    url: d.url || '', salaris_ind: '', gevonden_op: vandaag(),
